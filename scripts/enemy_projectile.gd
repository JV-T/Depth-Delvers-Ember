...
@export var max_distance: float = 800
...
func _on_body_entered(body: Node2D) -> void:
	if body.name == "miner":
		UserInterface.oxygen -= damage
		UserInterface.knockback = -5
		UserInterface.shakeamount += 30
		GlobalWorldEnvironment.get_node("zap").zapeffect()
		_spawn_impact_particles()
		queue_free()
	elif body.is_in_group("walls") or body is TileMap or body is TileMapLayer or body is StaticBody2D:
		_spawn_impact_particles()
		queue_free()
